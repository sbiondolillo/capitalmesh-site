...
single named user. We instead extended the existing `new-event-post` Claude Code skill so the
...
already exists (Claude Code, the `new-event-post` skill, this repo) and needs zero new services.
